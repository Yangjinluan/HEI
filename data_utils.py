...
def sample_neg_neighborhood(dataset, device, args):
    ...
    filename_neg_adjacent_list = f'data/{args.dataset}'+f'_{args.runs}_{args.neighbor_max}_neg.pth'
    torch.save(adjacent_list,filename_neg_adjacent_list)
...
def sample_neighborhood(dataset, device, args):
    ...
    adjacent_list = list(adjacent_list.values())
    adjacent_list = torch.vstack(adjacent_list)
    # print(adjacent_list.size())
    filename_adjacent_list = f'data/{args.dataset}'+f'_{args.runs}_{args.neighbor_max}.pth'
    torch.save(adjacent_list,filename_adjacent_list)
    return adjacent_list.to(device)
...
@torch.no_grad()
def evaluate(model, dataset, split_idx, test_max_idx, test_min_idx, eval_func, result=None, sampling=False, subgraph_loader=None):
    if result is not None:
        ...
        if not sampling:
            # out= model(dataset)
            # out,_,_ = model(dataset)
            _,out= model(dataset)
        else:
            # out = model.inference(dataset, subgraph_loader)
            # out,_,_ = model.inference(dataset, subgraph_loader)
            _,out = model.inference(dataset, subgraph_loader)
    # print(out.size())
    # print(dataset.label[split_idx['train']].size())
    # # print(1/0)
    train_acc = eval_func(
    ...
    test_acc_max = eval_func(
        dataset.label[test_max_idx], out[test_max_idx])
    test_acc_min = eval_func(
        dataset.label[test_min_idx], out[test_min_idx])

    return train_acc, valid_acc, test_acc, test_acc_max, test_acc_min, out

@torch.no_grad()
def evaluate_mlpnorm_hom(model, x, adj, dataset, split_idx, test_max_idx, test_min_idx, eval_func, result=None, sampling=False, subgraph_loader=None):
    if result is not None:
        out = result
    else:
        model.eval()
...
@torch.no_grad()
def evaluate_pro(model, dataset, split_idx, test_max_idx, test_min_idx, eval_func, result=None, sampling=False, subgraph_loader=None):
    if result is not None:
        out = result
    else:
        model.eval()
        if not sampling:
            out, min_distances = model(dataset)
        else:
            out = model.inference(dataset, subgraph_loader)
    # print(out.size())
    # print(dataset.label[split_idx['train']].size())
...
@torch.no_grad()
def evaluate_dssl(model, dataset, split_idx, test_max_idx, test_min_idx, eval_func):
    model.eval()
    # out = model.class_layer(model.online_encoder(dataset))
    out = model.online_encoder(dataset)
    train_acc = eval_func(
        dataset.label[split_idx['train']], out[split_idx['train']])
    valid_acc = eval_func(
        dataset.label[split_idx['valid']], out[split_idx['valid']])
