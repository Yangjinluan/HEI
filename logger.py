...
class Logger(object):
    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            print(f'   Final Test_Max: {result[argmax, 3]:.2f}')
            print(f'   Final Test_Min: {result[argmax, 4]:.2f}')
        else:
            # print(self.results.size())
            # print(self.results)
            result_list = []
            for index in self.results:
                result_list.append(100 * torch.tensor(index))
            best_results = []
            for r in result_list:
                ...
                test_max = r[r[:, 1].argmax(), 3].item()
                test_min = r[r[:, 1].argmax(), 4].item()
                best_results.append((train1, valid, train2, test, test_max, test_min))
            ...
            r = best_result[:, 4]
            print(f'   Final Test_Max: {r.mean():.2f} ± {r.std():.2f}')
            r = best_result[:, 5]
            print(f'   Final Test_Min: {r.mean():.2f} ± {r.std():.2f}')
