...
# distance_weight_lst=(0.01 0.05 0.1 0.3 0.5 1)
# lr_lst=(1e-3 5e-3 1e-2)
# weight_decay_lst=(1e-1 1e-2 5e-2 5e-3)

# warm_up_epochs_lst=(0 5 10 20)
# distance_weight_lst=(0.001 0.005 0.01 0.03 0.05 0.1 0.3 0.5 1)
# lr_lst=(5e-3 1e-3 5e-4)
# weight_decay_lst=(1e-2 5e-3 1e-3 5e-4)

warm_up_epochs_lst=(0 5)
distance_weight_lst=(0.005 0.01 0.02 0.03 0.05 0.1 0.2 0.3 0.5 0.8 1)
lr_lst=(5e-2 3e-2 1e-2 5e-3 1e-3)
weight_decay_lst=(1e-2 5e-3 1e-3 5e-4 1e-4)
                                        ...
                                        for alpha in "${alpha_lst[@]}"; do
                                            for delta in "${delta_lst[@]}"; do
                                                if [ "$dataset" = "snap-patents" ] || [ "$dataset" = "arxiv-year" ]; then
                                                    python -u main_glo.py --device "cuda:0" --dataset $dataset --sub_dataset ${sub_dataset:-''} --method mlpnorm --epochs $epochs --warm_up_epochs $warm_up_epochs    --distance_weight $distance_weight --prototype_loss  "global"   --hidden_channels $hidden_channels --lr $lr --dropout $dropout --weight_decay $weight_decay --alpha $alpha --beta $beta --gamma $gamma --delta $delta --norm_func_id $norm_func_id --norm_layers $norm_layers --orders_func_id $order_func_id --orders $orders --display_step 1 --runs $runs --directed
                                                else
